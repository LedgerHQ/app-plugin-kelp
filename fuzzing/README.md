...
In the case of `app-plugin-kelp` we want to test the code that is responsible for handling the contract data.
...
docker build -t app-plugin-kelp --file .clusterfuzzlite/Dockerfile .
```

### Compilation

```shell
docker run --rm --privileged -e FUZZING_LANGUAGE=c -v "$(realpath .)/fuzzing/out:/out" -ti app-plugin-kelp
```
